# LLM-OPTIMIZED REFERENCE -- code-review-navigator v2.3.6

AI coding agents: Read ONLY the exact `<section>` you need. Never load the whole file.

<section name="usage">
Quick install: pip install code-review-navigator
Then: code-review-navigator install && code-review-navigator build
First run: /code-review-navigator:build-graph
...
CLI: code-review-navigator [install|init|build|update|status|watch|visualize|serve|mcp|wiki|detect-changes|postprocess|embed|register|unregister|repos|eval|daemon]
...
<section name="legal">
MIT licence. Core graph/review workflows are local and there is no telemetry. DB file: .code-review-navigator/graph.db. Optional cloud embeddings send embedded source snippets to the configured provider only when selected.
</section>

<section name="watch">
Run: code-review-navigator watch (auto-updates graph on file save via watchdog)
Or use PostToolUse (Write|Edit|Bash) hooks for automatic background updates.
</section>

<section name="embeddings">
Optional: pip install "code-review-navigator[embeddings]"
...
Custom languages: add .code-review-navigator/languages.toml (extensions + node types per grammar) — no fork needed, see docs/CUSTOM_LANGUAGES.md. Built-ins cannot be overridden.
</section>

<section name="troubleshooting">
DB lock: SQLite WAL mode, auto-recovers. Only one build at a time.
Large repos: First build 30-60s. Incremental <2s. Add patterns to .code-review-navigatorignore.
Stale graph: Run /code-review-navigator:build-graph manually.
...
When user asks anything about "code-review-navigator", "how to use", "commands", "review-delta", etc.:

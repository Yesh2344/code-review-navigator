...
code-review-navigator detect-changes --brief --verify
```
...
| code-review-navigator | 47 | 539,206 | 134,820 | 120,760 | 1.116 |
...
- `nextjs.yaml` was a misnamed config evaluating this repo — renamed to `code-review-navigator.yaml`
...
```bash
git clone https://github.com/your-github-username/code-review-navigator
...
```bash
uv run code-review-navigator eval \
...
- `evaluate/test_repos/{express,fastapi,flask,gin,httpx,code-review-navigator}/`
- `evaluate/test_repos/<name>/.code-review-navigator/graph.db`
...
```bash
for repo in express fastapi flask gin httpx code-review-navigator; do
  uv run code-review-navigator embed --repo "evaluate/test_repos/$repo"
done
```
...
for repo in sorted(Path("evaluate/test_repos").iterdir()):
    db = repo / ".code-review-navigator" / "graph.db"
    if not db.exists():
        continue
...
| code-review-navigator | `84bde354` | 208,821 | 2,495 | **93.0×** |
...
|---|---|---|---:|---:|---:|
| code-review-navigator | crg-parse-file-callers | yes | 0 | 1.00 | **1.00** |
| code-review-navigator | crg-upsert-node-callers | yes | 4 | 1.00 | **1.00** |
...
| code-review-navigator | 1,418 | 8,877 | 104 | 11 | 1,326 | 38 |

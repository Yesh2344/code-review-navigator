# FAQ — how code-review-navigator compares
...
Language servers and code-review-navigator (CRG) both build a structural model of your
...
- **One persistent graph instead of per-language daemons.** Language servers run one
  ...
  edges in a single SQLite file (`.code-review-navigator/graph.db`), and answers queries
...
|---|---|---|---|---|
| **code-review-navigator** | Tree-sitter AST → structural graph (calls, imports, inheritance, tests) over MCP + CLI | SQLite in `.code-review-navigator/`, incremental updates | None for the core; embeddings optional | Yes — blast radius, risk-scored change analysis, test-gap detection |
...
No. There is zero telemetry. The graph is a SQLite file in `.code-review-navigator/`
...
- **Local embeddings** (`pip install "code-review-navigator[embeddings]"`) download the
...
2. **See the savings on a real change** — make any edit, then:
   ...
   ```bash
   code-review-navigator detect-changes --brief
   ```
   ...
   the graph is stale, `code-review-navigator update --brief` re-parses changed files
   first and prints the same panel.

3. **Check the MCP wiring** — in Claude Code, run `/mcp` and confirm the
   `code-review-navigator` server is connected with its tools listed. Then ask the
...
Use a `.code-review-navigatorignore` file to exclude tracked paths (e.g. `vendor/**`,
...
`.code-review-navigator/` database matching its checkout. Don't try to share one database
...
`~/.code-review-navigator/registry.json`) lets MCP clients search across projects:

```bash
code-review-navigator register ~/work/api --alias api   # add a repo (optional alias)
code-review-navigator repos                             # list registered repos
code-review-navigator unregister api                    # remove by path or alias
```
...
(Also available as `code-review-navigator daemon start|stop|status`.) See

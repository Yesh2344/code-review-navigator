...
code-review-navigator ships a composite GitHub Action (`action.yml` at the repo
...
1. Installs `code-review-navigator` from PyPI.
2. Restores the cached `.code-review-navigator/` SQLite graph (or builds it from
   scratch on a cache miss) and incrementally re-parses the files changed by
   the PR.
3. Runs `code-review-navigator detect-changes --base origin/<base-branch>` to get
...
```yaml
# .github/workflows/code-review-navigator.yml
...
jobs:
  review:
    ...
    steps:
      ...
      - uses: your-github-username/code-review-navigator@v2.3.6
        with:
...
| `python-version` | no | `3.12` | Python version used to run code-review-navigator (3.10+ supported). |
...
- A `Powered by code-review-navigator` footer.

The comment starts with a hidden HTML marker
(`<!-- code-review-navigator-report -->`). The action looks the marker up via
`gh api` on each run and PATCHes the existing comment instead of creating a
new one (a "sticky" comment).

## Cache behavior

The action caches the `.code-review-navigator/` directory (the SQLite graph
database) with `actions/cache`:

- **Key**: `code-review-navigator-schema9-<runner.os>-<hashFiles(lockfiles)>`,
...
- **On cache hit**: the action runs `code-review-navigator update --base
...
- **On cache miss**: a full `code-review-navigator build` runs (one-time cost;
...
```bash
code-review-navigator detect-changes --base origin/main | \

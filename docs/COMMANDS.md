...
### `/code-review-navigator:review-pr`
...
provider: str | None # local, openai, google, minimax
```
Local embeddings require: `pip install "code-review-navigator[embeddings]"`. Cloud providers use stdlib HTTP clients and require their provider environment variables.
...
```bash
# Setup
code-review-navigator install           # Configure detected AI coding platforms (alias: init)
code-review-navigator install --dry-run # Preview without writing files
code-review-navigator install --platform codex  # Configure one platform

# Build and update
code-review-navigator build                        # Full build
code-review-navigator build --skip-flows           # Parse + signatures + FTS only
code-review-navigator build --skip-postprocess     # Raw parse only
code-review-navigator update                       # Incremental update
code-review-navigator update --base origin/main    # Custom base ref
code-review-navigator update --brief               # Update graph + show risk panel
code-review-navigator update --brief --verify      # ...and cross-check vs tiktoken
code-review-navigator postprocess                  # Re-run flows, communities, FTS
code-review-navigator embed --provider local       # Compute vector embeddings for semantic search
code-review-navigator update --embedding-provider local --embedding-model all-MiniLM-L6-v2
                                                # Explicitly refresh an existing index (default: off)

# Monitor and inspect
code-review-navigator status                       # Graph statistics
code-review-navigator watch                        # Auto-update on file changes
code-review-navigator visualize                    # Generate interactive HTML graph
code-review-navigator visualize --format graphml   # Export GraphML
code-review-navigator visualize --serve            # Serve graph.html on localhost:8765

# Analysis
code-review-navigator detect-changes               # Risk-scored change analysis
code-review-navigator detect-changes --base HEAD~3 # Custom base ref
code-review-navigator detect-changes --brief       # Compact panel with token-savings estimate
code-review-navigator detect-changes --brief --verify  # ...and cross-check vs tiktoken
code-review-navigator detect-changes --churn       # Add opt-in change-frequency risk
...
# Wiki
code-review-navigator wiki                         # Generate markdown wiki from communities

# Multi-repo
code-review-navigator register <path> [--alias name]  # Register a repository
code-review-navigator unregister <path_or_alias>       # Remove from registry
code-review-navigator repos                            # List registered repositories

# Daemon (multi-repo watcher) — included with install, no extra dependencies
code-review-navigator daemon start [--foreground]       # Start the watch daemon
code-review-navigator daemon stop                       # Stop the daemon
code-review-navigator daemon restart [--foreground]     # Restart the daemon
code-review-navigator daemon status                     # Show daemon status and repos
code-review-navigator daemon logs [--repo ALIAS] [--follow]  # View daemon or per-repo logs
code-review-navigator daemon add <path> [--alias NAME]  # Add a repo to daemon config
code-review-navigator daemon remove <path_or_alias>     # Remove a repo from daemon config

# Evaluation
code-review-navigator eval                         # Run evaluation benchmarks

# Server
code-review-navigator serve                        # Start MCP server (stdio)
code-review-navigator serve --http                 # Streamable HTTP on localhost:5555
code-review-navigator serve --tools query_graph_tool,detect_changes_tool  # Tool allowlist
code-review-navigator mcp                          # Alias for serve
```

## Standalone Daemon CLI (`crg-daemon`)

The `crg-daemon` command is included with every `code-review-navigator` installation — no
separate install required. It is also available as a standalone entry point. It mirrors the
`code-review-navigator daemon` subcommands:
...
The daemon reads its configuration from `~/.code-review-navigator/watch.toml`:

```toml
session_name = "crg-watch"   # logical daemon name
log_dir = "~/.code-review-navigator/logs"
...
The daemon spawns one `code-review-navigator watch` child process per repo,

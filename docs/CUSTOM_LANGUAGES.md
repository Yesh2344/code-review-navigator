...
code-review-navigator ships parsers for 30+ languages, but the
...
Create `<repo_root>/.code-review-navigator/languages.toml`:
...
```bash
uv run code-review-navigator build
```
...
- The config is read when a parser is constructed (every `build`/`update`),
  ...
  `uv run code-review-navigator build` after editing.

...
### v2.3.6
- **Custom languages without forking**: `.code-review-navigator/languages.toml` maps extensions and node types to any tree-sitter-language-pack grammar (`docs/CUSTOM_LANGUAGES.md`)
...
- **`code-review-navigator embed`** CLI subcommand for explicit embedding generation
...
### v2.2.0
- Multi-repo watch daemon (`crg-daemon` / `code-review-navigator daemon`)
- TOML-based daemon configuration (`~/.code-review-navigator/watch.toml`)
- Child process management: one `code-review-navigator watch` process per repo
...
### v1.5.x
- Generated files in `.code-review-navigator/` directory

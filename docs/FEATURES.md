...
- **Custom languages without forking**: drop a `.code-review-navigator/languages.toml` into your repo to index any grammar shipped by tree-sitter-language-pack — extension map plus node-type lists, validated and capped, with built-in languages always winning. See [CUSTOM_LANGUAGES.md](CUSTOM_LANGUAGES.md).
...
## v2.3.5
- **Token Savings panel on every brief CLI call**: `code-review-navigator detect-changes --brief` and the new `code-review-navigator update --brief` print a boxed `Token Savings` panel — full-context baseline, graph response, saved tokens, percent, and per-category breakdown (Functions / Tests / Risk / Other) that sums exactly to the graph response size.
...
- **`code-review-navigator embed` CLI subcommand**: explicit shell-level access to embedding generation. Previously only reachable via MCP.
...
## v1.7.0
- **`install` command**: New primary entry point for setup (`code-review-navigator install`). `init` remains as an alias.
...
- **File organization**: Generated files moved into `.code-review-navigator/` directory (auto-created `.gitignore`, legacy migration)
...
- **Interactive D3.js graph visualization**: `code-review-navigator visualize` generates an HTML graph you can explore in-browser
...
- **Universal install**: `pip install code-review-navigator` — no git clone needed
- **CLI entry point**: `code-review-navigator` command available system-wide after pip install
...
## v1.1.0
- **Watch mode**: `code-review-navigator watch` — auto-rebuilds graph on file changes
...
- Graph stored in `.code-review-navigator/graph.db` (SQLite), auto-gitignored
...
- Respects `.gitignore` and `.code-review-navigatorignore`

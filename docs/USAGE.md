...
```bash
pip install code-review-navigator
code-review-navigator install    # auto-detects and configures all supported platforms
code-review-navigator build      # parse your codebase
```
...
```bash
code-review-navigator install --platform codex
code-review-navigator install --platform cursor
code-review-navigator install --platform claude-code
code-review-navigator install --platform codebuddy
```
...
open .code-review-navigator/graph.html
```
...
```bash
pip install "code-review-navigator[embeddings]"
```
...
```
Creates markdown wiki pages for each detected community in `.code-review-navigator/wiki/`.

### 10. Multi-repo search (v2)
```bash
code-review-navigator register /path/to/other/repo --alias mylib
```
...
```bash
code-review-navigator eval --all
```
...
Languages not covered yet can be added without a fork via a `.code-review-navigator/languages.toml` config — see [CUSTOM_LANGUAGES.md](CUSTOM_LANGUAGES.md).
...
```
.code-review-navigator/**    node_modules/**    .git/**
...
To add custom patterns, create a `.code-review-navigatorignore` file in your repo root (same syntax as `.gitignore`):
...
In git repos, indexing is based on tracked files (`git ls-files`), so gitignored files are skipped automatically. Use `.code-review-navigatorignore` to exclude tracked files or when git isn't available.

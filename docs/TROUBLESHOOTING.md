...
```bash
pip install --upgrade code-review-navigator   # → v2.2.4 or later
cd /path/to/your/project
code-review-navigator install                 # rewrites .claude/settings.json
```
...
### 2. `code-review-navigator: command not found` after `pip install`
...
```bash
pip uninstall code-review-navigator
pipx install code-review-navigator
```
...
```bash
uvx code-review-navigator install
uvx code-review-navigator build
```
...
```bash
pip show code-review-navigator | grep Location
...
### 3. Is code-review-navigator project-scoped or user-scoped?
...
| The graph database            | Project-scoped | `.code-review-navigator/graph.db` inside each project                |
| MCP server config (`.mcp.json`) | Project-scoped | Claude Code launches one MCP server per project, with `cwd=<project>` |
| Multi-repo registry           | User-scoped    | `~/.code-review-navigator/registry.json` (only for `cross_repo_search`) |

**TL;DR**: install the tool **once**, then run `code-review-navigator install && code-review-navigator build` inside **each** project you want graph-aware reviews in.

### 4. Using a venv? You must update `settings.json` manually

Claude Code hooks and MCP tool paths in `.claude/settings.json` are **hardcoded at install time**. If you switch to (or create) a virtual environment after running `code-review-navigator install`, the paths will still point to the old interpreter and the server will silently fail or use the wrong Python.
...
Or simply re-run `code-review-navigator install` **from within the activated venv** so the paths are regenerated correctly:

```bash
source .venv/bin/activate          # activate your venv first
code-review-navigator install          # rewrites .mcp.json and hook paths
```
...
2. **New session's `cwd` is a different directory.** The MCP server is launched with `cwd=<project>` and it reads `.code-review-navigator/graph.db` from there. If your new session opened in a parent folder or a different project, it won't find the graph you built.
...
```bash
cd /path/to/your/project
code-review-navigator status    # should print Files/Nodes/Edges from the built graph
ls .mcp.json                # should exist
cat .mcp.json               # should reference `code-review-navigator serve`
# then: fully quit Claude Code and reopen it inside this project
```

If `status` shows the graph but `/mcp` in the new session doesn't list `code-review-navigator`, the `.mcp.json` isn't in the session's `cwd` — re-run `code-review-navigator install` from the correct project root.
...
- Delete `.code-review-navigator/graph.db-wal` and `.code-review-navigator/graph.db-shm` if corrupt

## Large repositories (>10k files)
- First build may take 30-60 seconds
- Subsequent incremental updates are fast (<2s)
- Add more ignore patterns to `.code-review-navigatorignore`:
  ```
  generated/**
  vendor/**
  *.min.js
...
- If stale, run `/code-review-navigator:build-graph` manually
- Check that hooks are configured in `.claude/settings.json` (re-run `code-review-navigator install` to regenerate)

## Embeddings not working
- Install with: `pip install "code-review-navigator[embeddings]"`
...
- Check that `uvx code-review-navigator serve` runs without errors
- If using a custom `.mcp.json`, ensure it uses `"command": "uvx"` with `"args": ["code-review-navigator", "serve"]`
- Re-run `code-review-navigator install` to regenerate the config
...
- File watching (`code-review-navigator watch`) may have delays on WSL1 due to filesystem event limitations; WSL2 is recommended
...
- Install with: `pip install "code-review-navigator[communities]"`
...
- Install with: `pip install "code-review-navigator[wiki]"`
...
- `pip install "code-review-navigator[embeddings]"` for semantic search
- `pip install "code-review-navigator[google-embeddings]"` for Google Gemini embeddings
- OpenAI-compatible and MiniMax embeddings use stdlib HTTP clients and require only their environment variables
- `pip install "code-review-navigator[communities]"` for igraph-based community detection
- `pip install "code-review-navigator[enrichment]"` for Python call-resolution enrichment via Jedi
- `pip install "code-review-navigator[eval]"` for evaluation benchmarks (matplotlib)
- `pip install "code-review-navigator[wiki]"` for wiki LLM summaries (ollama)
- `pip install "code-review-navigator[all]"` for everything

#!/bin/bash
# Checks for the code-review-navigator knowledge graph and outputs
...
DB_PATH=".code-review-navigator/graph.db"

if [ -f "$DB_PATH" ]; then
    cat <<'INSTRUCTIONS'
[code-review-navigator] Knowledge graph is available.

When answering questions about this codebase, prefer using the code-review-navigator MCP tools before scanning files manually:
...
INSTRUCTIONS
    ...
    echo "[code-review-navigator] No knowledge graph found. Run /code-review-navigator:build-graph to parse this codebase and enable graph-powered queries."
fi

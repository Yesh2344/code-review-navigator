...
AIコーディングツールはレビュータスクでコードベースの大きな範囲を読み直しがちです。`code-review-navigator` はその問題を解決します。[Tree-sitter](https://tree-sitter.github.io/tree-sitter/) でコードの構造マップを構築し、変更を差分で追跡し、[MCP](https://modelcontextprotocol.io/) を通じてAIアシスタントに必要最小限のコンテキストだけを提供します。
...
```bash
pip install code-review-navigator                     # または: pipx install code-review-navigator
code-review-navigator install          # 対応プラットフォームを自動検出して設定
code-review-navigator build            # コードベースを解析
```
...
```bash
code-review-navigator install --platform codex       # Codexのみ設定
code-review-navigator install --platform cursor      # Cursorのみ設定
code-review-navigator install --platform claude-code  # Claude Codeのみ設定
code-review-navigator install --platform kiro         # Kiroのみ設定
```

Python 3.10以上が必要です。最良の体験のためには [uv](https://docs.astral.sh/uv/) のインストールを推奨します（MCP設定は利用可能な場合 `uvx` を使用し、そうでない場合は `code-review-navigator` コマンドに直接フォールバックします）。
...
<p align="center">
  <img src="diagrams/diagram6_monorepo_funnel.png" alt="Next.jsモノレポ：27,732ファイルをcode-review-navigatorで絞り込み、約15ファイルに - トークン49分の1" width="80%" />
</p>
...
すべての数値は6つの実際のオープンソースリポジトリ（合計13コミット）に対する自動評価ランナーの結果です。`code-review-navigator eval --all` で再現可能です。完全な再現手順と正規の数値は [`docs/REPRODUCING.md`](docs/REPRODUCING.md) をご覧ください。
...
| **ローカルストレージ** | `.code-review-navigator/` 内のSQLiteファイル。コアのグラフ保存に外部DBやクラウドサービスは不要。 |
...
| コマンド | 説明 |
|---------|------|
| `/code-review-navigator:build-graph` | コードグラフのビルドまたは再ビルド |
| `/code-review-navigator:review-delta` | 最後のコミット以降の変更をレビュー |
| `/code-review-navigator:review-pr` | 影響範囲分析付きのフルPRレビュー |
...
```bash
code-review-navigator install          # 全プラットフォームを自動検出して設定
code-review-navigator install --platform <name>  # 特定のプラットフォームのみ設定
code-review-navigator build            # コードベース全体を解析
code-review-navigator update           # インクリメンタル更新（変更ファイルのみ）
code-review-navigator status           # グラフの統計情報
code-review-navigator watch            # ファイル変更時に自動更新
code-review-navigator visualize        # インタラクティブHTMLグラフを生成
code-review-navigator visualize --format graphml   # GraphML形式でエクスポート
code-review-navigator visualize --format svg       # SVG形式でエクスポート
code-review-navigator visualize --format obsidian  # Obsidianボールトとしてエクスポート
code-review-navigator visualize --format cypher    # Neo4j Cypher形式でエクスポート
code-review-navigator wiki             # コミュニティからMarkdown Wikiを生成
code-review-navigator detect-changes   # リスクスコア付き変更影響分析
code-review-navigator register <path>  # マルチリポジトリレジストリにリポジトリを登録
code-review-navigator unregister <id>  # レジストリからリポジトリを削除
code-review-navigator repos            # 登録済みリポジトリの一覧表示
code-review-navigator eval             # 評価ベンチマークの実行
code-review-navigator serve            # MCPサーバーの起動
```
...
インデックス対象から除外するパスを指定するには、リポジトリルートに `.code-review-navigatorignore` ファイルを作成します：
...
注意：gitリポジトリでは追跡対象ファイルのみがインデックスされます（`git ls-files`）。そのため、gitignoreされたファイルは自動的にスキップされます。`.code-review-navigatorignore` は追跡対象ファイルの除外や、gitが利用できない環境で使用してください。

オプションの依存グループ：

```bash
pip install "code-review-navigator[embeddings]"          # ローカルベクトル埋め込み (sentence-transformers)
pip install "code-review-navigator[google-embeddings]"   # Google Gemini埋め込み
pip install "code-review-navigator[communities]"         # コミュニティ検出 (igraph)
pip install "code-review-navigator[enrichment]"          # Python呼び出し解決の補強 (Jedi)
pip install "code-review-navigator[eval]"                # 評価ベンチマーク (matplotlib)
pip install "code-review-navigator[wiki]"                # LLMサマリー付きWiki生成 (ollama)
pip install "code-review-navigator[all]"                 # 全オプション依存
```
...
>
> また注意：現状 `code-review-navigator` は**関数シグネチャのみ**を埋め込みます（ノードあたり約10トークン、例：`"parse_file function (path: str) returns Tree"`）。長 context で関数 body を理解する能力で差をつけるモデル（Gemini 2 や Qwen3-8B の MTEB-code SOTA スコア）は、この入力長では小型モデルとの差がかなり縮まります。Body / docstring 埋め込みはフォローアップ拡張として追跡中です。

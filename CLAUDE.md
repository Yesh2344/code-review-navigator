...
- **Core Package**: `code_review_graph/` (Python 3.10+)
  ...
  - `custom_languages.py` — Config-driven custom language support (`.code-review-navigator/languages.toml`, see docs/CUSTOM_LANGUAGES.md)
...
- **VS Code Extension**: `code-review-navigator-vscode/` (TypeScript)
  - Separate subproject with its own `package.json`, `tsconfig.json`
  - Reads from `.code-review-navigator/graph.db` via SQLite

- **Database**: `.code-review-navigator/graph.db` (SQLite, WAL mode)
...
# Build & test
uv run code-review-navigator build              # Full graph build
uv run code-review-navigator update             # Incremental update
uv run code-review-navigator status             # Show stats
uv run code-review-navigator serve              # Start MCP server
uv run code-review-navigator wiki               # Generate markdown wiki
uv run code-review-navigator detect-changes     # Risk-scored change analysis
uv run code-review-navigator register <path>    # Register repo in multi-repo registry
uv run code-review-navigator repos              # List registered repos
uv run code-review-navigator eval               # Run evaluation benchmarks
```
...
<!-- code-review-navigator MCP tools -->

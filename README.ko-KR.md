...
AI 코딩 도구는 리뷰 작업에서 코드베이스의 큰 부분을 반복해서 읽게 될 수 있습니다. `code-review-navigator`는 이 문제를 해결합니다. [Tree-sitter](https://tree-sitter.github.io/tree-sitter/)로 코드의 구조적 맵을 구축하고, 변경 사항을 점진적으로 추적하며, [MCP](https://modelcontextprotocol.io/)를 통해 AI 어시스턴트에게 정확한 컨텍스트를 제공하여 필요한 부분만 읽도록 합니다.
...
```bash
pip install code-review-navigator                     # 또는: pipx install code-review-navigator
code-review-navigator install          # 지원되는 모든 플랫폼을 자동 감지하고 설정
code-review-navigator build            # 코드베이스 파싱
```
...
```bash
code-review-navigator install --platform codex       # Codex만 설정
code-review-navigator install --platform cursor      # Cursor만 설정
code-review-navigator install --platform claude-code  # Claude Code만 설정
code-review-navigator install --platform kiro         # Kiro만 설정
```

Python 3.10 이상이 필요합니다. 최상의 경험을 위해 [uv](https://docs.astral.sh/uv/)를 설치하세요 (MCP 설정은 `uvx`가 있으면 이를 사용하고, 없으면 `code-review-navigator` 명령을 직접 사용합니다).
...
<p align="center">
  <img src="diagrams/diagram6_monorepo_funnel.png" alt="Next.js 모노레포: 27,732개 파일이 code-review-navigator를 거쳐 약 15개 파일로 -- 49배 적은 토큰" width="80%" />
</p>
...
모든 수치는 6개 실제 오픈소스 저장소(총 13개 커밋)에 대한 자동화된 평가 실행 결과입니다. `code-review-navigator eval --all`로 재현할 수 있습니다. 전체 재현 절차와 기준 수치는 [`docs/REPRODUCING.md`](docs/REPRODUCING.md)에 있습니다.
...
| **로컬 스토리지** | `.code-review-navigator/`에 SQLite 파일 저장. 핵심 그래프 저장에는 외부 데이터베이스나 클라우드 서비스가 필요 없습니다. |
...
| 명령 | 설명 |
|------|------|
| `/code-review-navigator:build-graph` | 코드 그래프 빌드 또는 재빌드 |
| `/code-review-navigator:review-delta` | 마지막 커밋 이후 변경 사항 리뷰 |
| `/code-review-navigator:review-pr` | 영향 범위 분석을 포함한 전체 PR 리뷰 |
...
```bash
code-review-navigator install          # 모든 플랫폼 자동 감지 및 설정
code-review-navigator install --platform <name>  # 특정 플랫폼 지정
code-review-navigator build            # 전체 코드베이스 파싱
code-review-navigator update           # 점진적 업데이트 (변경 파일만)
code-review-navigator status           # 그래프 통계
code-review-navigator watch            # 파일 변경 시 자동 업데이트
code-review-navigator visualize        # 인터랙티브 HTML 그래프 생성
code-review-navigator visualize --format graphml   # GraphML로 내보내기
code-review-navigator visualize --format svg       # SVG로 내보내기
code-review-navigator visualize --format obsidian  # Obsidian vault로 내보내기
code-review-navigator visualize --format cypher    # Neo4j Cypher로 내보내기
code-review-navigator wiki             # 커뮤니티에서 마크다운 위키 생성
code-review-navigator detect-changes   # 위험 점수 기반 변경 영향 분석
code-review-navigator register <path>  # 멀티 레포 레지스트리에 저장소 등록
code-review-navigator unregister <id>  # 레지스트리에서 저장소 제거
code-review-navigator repos            # 등록된 저장소 목록
code-review-navigator eval             # 평가 벤치마크 실행
code-review-navigator serve            # MCP 서버 시작
```
...
인덱싱에서 경로를 제외하려면 저장소 루트에 `.code-review-navigatorignore` 파일을 생성하세요:
...
참고: git 저장소에서는 추적되는 파일만 인덱싱됩니다 (`git ls-files`). gitignore된 파일은 자동으로 건너뜁니다. `.code-review-navigatorignore`는 추적되는 파일을 제외하거나 git을 사용할 수 없을 때 사용합니다.

선택적 의존성 그룹:

```bash
pip install "code-review-navigator[embeddings]"          # 로컬 벡터 임베딩 (sentence-transformers)
pip install "code-review-navigator[google-embeddings]"   # Google Gemini 임베딩
pip install "code-review-navigator[communities]"         # 커뮤니티 감지 (igraph)
pip install "code-review-navigator[enrichment]"          # Python 호출 해결 보강 (Jedi)
pip install "code-review-navigator[eval]"                # 평가 벤치마크 (matplotlib)
pip install "code-review-navigator[wiki]"                # LLM 요약 위키 생성 (ollama)
pip install "code-review-navigator[all]"                 # 모든 선택적 의존성
```
...
>
> 참고로 현재 `code-review-navigator`는 **함수 시그니처만** 임베딩합니다(노드당 약 10 토큰, 예: `"parse_file function (path: str) returns Tree"`). 긴 context로 함수 body를 이해하는 능력으로 차별화되는 모델(Gemini 2 또는 Qwen3-8B의 MTEB-code SOTA 점수)은 이 입력 길이에서 소형 모델과의 품질 차이가 훨씬 좁아집니다. Body / docstring 임베딩은 후속 개선 과제로 추적 중입니다.

...
dropping a ``languages.toml`` file into ``.code-review-navigator/``::
...
CONFIG_RELATIVE_PATH = Path(".code-review-navigator") / "languages.toml"
...
def load_custom_languages(
    ...
    *,
    ...
    """Load and validate ``<repo_root>/.code-review-navigator/languages.toml``.

    Returns a mapping of custom language name -> :class:`CustomLanguage`.
    Always returns (possibly empty) — a broken config never raises.

    Args:
        repo_root: Repository root containing ``.code-review-navigator/``.

...
Manages a registry of multiple repositories at ``~/.code-review-navigator/registry.json``
...
_REGISTRY_DIR = Path.home() / ".code-review-navigator"
_REGISTRY_PATH = _REGISTRY_DIR / "registry.json"


class Registry:
    """Manages a JSON-based registry of code-review-navigator repositories.

    Each entry stores the repo path and an optional alias.
    The registry lives at ``~/.code-review-navigator/registry.json``.
    """
    ...
    def register(
        ...
        Validates that the path contains a ``.git`` or ``.code-review-navigator``
        directory.
        ...
        if not has_repo_marker:
            raise ValueError(
                ...
                f"(no .git, .svn, or .code-review-navigator): {resolved}"
            )

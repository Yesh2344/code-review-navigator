...
def enrich_search(pattern: str, repo_root: str) -> str:
    ...
    db_path = Path(repo_root) / ".code-review-navigator" / "graph.db"
    ...
    try:
        ...
        header = f'[code-review-navigator] {count} symbol(s) matching "{pattern}":\n'
        return header + "\n".join(all_lines)
    finally:
        store.close()
...
def enrich_file_read(file_path: str, repo_root: str) -> str:
    ...
    try:
        ...
        header = (
            f"[code-review-navigator] {len(interesting)} symbol(s) in {rel_path}:\n"
        )
        return header + "\n".join(all_lines)
    finally:
        store.close()

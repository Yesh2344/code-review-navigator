"""Multi-repo watch daemon for code-review-navigator.

Reads ``~/.code-review-navigator/watch.toml`` to configure which repositories
to watch, then spawns one ``code-review-navigator watch`` child process per
...
CONFIG_PATH: Path = Path.home() / ".code-review-navigator" / "watch.toml"
PID_PATH: Path = Path.home() / ".code-review-navigator" / "daemon.pid"
STATE_PATH: Path = Path.home() / ".code-review-navigator" / "daemon-state.json"
...
class DaemonConfig:
    ...
    log_dir: Path = field(default_factory=lambda: Path.home() / ".code-review-navigator" / "logs")
...
def load_config(path: Path | None = None) -> DaemonConfig:
    ...
    for entry in raw.get("repos", []):
        ...
        has_repo_marker = (
            ...
            or (repo_path / ".code-review-navigator").exists()
        )
        if not has_repo_marker:
            logger.warning(
                "Skipping repo %s — no .git, .svn, or .code-review-navigator directory found",
                repo_path,
            )
            continue
...
def add_repo_to_config(
    ...
    has_repo_marker = (
        ...
        or (resolved / ".code-review-navigator").exists()
    )
    if not has_repo_marker:
        raise ValueError(f"No .git, .svn, or .code-review-navigator directory in {resolved}")
...
class WatchDaemon:
    ...
    Each watched repository gets a ``code-review-navigator watch`` child process
    ...
    def start(self) -> None:
        ...
        for repo in self._config.repos:
            db_path = Path(repo.path) / ".code-review-navigator" / "graph.db"
    ...
    def _start_watcher(self, repo: WatchRepo) -> None:
        """Spawn a child process running ``code-review-navigator watch`` for *repo*."""
        self._config.log_dir.mkdir(parents=True, exist_ok=True)
        log_path = self._config.log_dir / f"{repo.alias}.log"

        crg_bin = shutil.which("code-review-navigator")
        if crg_bin:

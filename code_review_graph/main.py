...
Run as: code-review-navigator serve
Communicates via stdio (standard MCP transport), or use
``code-review-navigator serve --http`` for Streamable HTTP on localhost (port 5555
by default).
"""
...
def _resolve_repo_root(repo_root: Optional[str]) -> Optional[str]:
    ...
    2. ``--repo`` CLI flag passed to ``code-review-navigator serve``
...
async def embed_graph_tool(
    ...
    Requires: pip install code-review-navigator[embeddings] (local provider only;
...
async def generate_wiki_tool(
    ...
    Pages are written to .code-review-navigator/wiki/ inside the repository.
...
def list_repos_tool() -> dict:
    ...
    Returns the list of repos registered at ~/.code-review-navigator/registry.json.
...
def _apply_tool_filter(tools: str | None = None) -> None:
    ...
    Example::
        ...
        # via CLI
        code-review-navigator serve --tools query_graph_tool,semantic_search_nodes_tool

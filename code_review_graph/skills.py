...
CLAUDE.md integration for seamless code-review-navigator usage.
...
def _warn_legacy_opencode_config(repo_root: Path) -> None:
    ...
    if isinstance(servers, dict) and "code-review-navigator" in servers:
        print(
...
def install_platform_configs(
    ...
    for key, plat in platforms_to_install.items():
        ...
        if plat["format"] == "array":
            ...
            if any(isinstance(s, dict) and s.get("name") == "code-review-navigator" for s in arr):
                print(f"  {plat['name']}: already configured in {config_path}")
                _record_configured(key, plat)
                continue
            arr_entry = {"name": "code-review-navigator", **server_entry}
            arr.append(arr_entry)
            existing[server_key] = arr
        else:
            servers = existing.get(server_key, {})
            if "code-review-navigator" in servers:
                print(f"  {plat['name']}: already configured in {config_path}")
                _record_configured(key, plat)
                continue
            servers["code-review-navigator"] = server_entry
...
_SKILLS: dict[str, dict[str, str]] = {
    "explore-codebase.md": {
        ...
        "body": (
            "## Explore Codebase\n\n"
            "Use the code-review-navigator MCP tools to explore and understand the codebase.\n\n"
            "### Steps\n\n"
                    ...
                    "hooks": [
                        {
                            ...
                            "command": (
                                "cat >/dev/null || true; "
                                "command -v code-review-navigator >/dev/null 2>&1 || exit 0; "
                                "git rev-parse --git-dir >/dev/null 2>&1"
                                " && code-review-navigator update --skip-flows"
                    ...
                    "hooks": [
                        {
                            ...
                            "timeout": 30,
                            "statusMessage": "Updating code-review-navigator",
                        },
                    ],
                },
            ],
                    ...
                    "hooks": [
                        {
                            ...
                            "timeout": 10,
                            "statusMessage": "Checking code-review-navigator status",
                        },
                    ],
                },
            ],
...
def install_git_hook(repo_root: Path) -> Path | None:
    ...
    Called automatically by ``code-review-navigator install``.
    ...
    ``code-review-navigator`` commands into that manager manually instead.
    ...
    script = """\
#!/bin/sh
# Installed by code-review-navigator. Remove this file to disable pre-commit graph checks.
if command -v code-review-navigator >/dev/null 2>&1; then
    code-review-navigator update || true
    code-review-navigator detect-changes --brief || true
fi
"""
    marker = "code-review-navigator detect-changes"
...
def install_codex_hooks(repo_root: Path) -> Path:
    ...
    Merges code-review-navigator hook entries into any existing hooks.json,
...
_CLAUDE_MD_SECTION_MARKER = "<!-- code-review-navigator MCP tools -->"

_CLAUDE_MD_SECTION = f"""{_CLAUDE_MD_SECTION_MARKER}
## MCP Tools: code-review-navigator

**IMPORTANT: This project has a knowledge graph. ALWAYS use the
code-review-navigator MCP tools BEFORE using Grep/Glob/Read to explore
...
applyTo: '**'
description: >-
  Use code-review-navigator MCP tools for token-efficient
...
**IMPORTANT: This project has a knowledge graph. ALWAYS use the
code-review-navigator MCP tools BEFORE using file/search tools to
...
_PLATFORM_INSTRUCTION_CUSTOM_SECTIONS: dict[str, tuple[str, str]] = {
    ".github/code-review-navigator.instruction.md": (_CLAUDE_MD_SECTION_MARKER, _COPILOT_SECTION),
}
...
_PLATFORM_INSTRUCTION_FILES: dict[str, tuple[str, ...]] = {
    ...
    "QODER.md": ("qoder",),
    ".kiro/steering/code-review-navigator.md": ("kiro",),
    ".github/code-review-navigator.instruction.md": ("copilot", "copilot-cli"),
...
#!/usr/bin/env bash
# code-review-navigator: session start status (Gemini CLI hook)
# Must output ONLY JSON on stdout. Logs go to stderr. Never blocks the session.
set -euo pipefail

cat > /dev/null || true

msg="$(code-review-navigator status --repo "__CRG_REPO__" 2>&1 | head -n 1 || true)"
...
"""
    ...
    update_script = """\
#!/usr/bin/env bash
# code-review-navigator: incremental update after write/replace (Gemini CLI hook)
# Must output ONLY JSON on stdout. Low-noise: no systemMessage.
set -euo pipefail

cat > /dev/null || true

code-review-navigator update --skip-flows --repo "__CRG_REPO__" >/dev/null 2>&1 || true
...
"""
    ...
    _ensure_group(
        ...
        name="code-review-navigator status",
        timeout=10_000,
    )
    _ensure_group(
        ...
        name="code-review-navigator update",
        timeout=30_000,
    )
...
def _cursor_hook_scripts() -> dict[str, str]:
    ...
    - crg-update.sh: runs ``code-review-navigator update --skip-flows`` after file edits
    - crg-session-start.sh: runs ``code-review-navigator status`` on session start
    - crg-pre-commit.sh: runs ``code-review-navigator detect-changes --brief`` before
      git commit commands
    ...
    update_script = """\
#!/usr/bin/env bash
# code-review-navigator: auto-update graph after file edits (Cursor hook)
...
output=$(code-review-navigator update --skip-flows 2>&1) || true
...
"""
    ...
    session_start_script = """\
#!/usr/bin/env bash
# code-review-navigator: show graph status on session start (Cursor hook)
# Fails gracefully — never blocks the editor.
set -euo pipefail
...
output=$(code-review-navigator status 2>&1) || output="graph not built yet"
...
"""
    ...
    pre_commit_script = """\
#!/usr/bin/env bash
# code-review-navigator: detect changes before git commit (Cursor hook)
# Fails gracefully — never blocks the editor.
set -euo pipefail
...
output=$(code-review-navigator detect-changes --brief 2>&1) || output=""
...
def install_cursor_hooks() -> Path:
    ...
    Writes ``~/.cursor/hooks.json`` (merging code-review-navigator hooks
...
def _opencode_plugin_content() -> str:
    ...
    1. ``file.edited`` — runs ``code-review-navigator update --skip-flows``
    2. ``session.created`` — runs ``code-review-navigator status``
    3. ``tool.execute.before`` — when the tool is a shell command starting
       with ``git commit``, runs ``code-review-navigator detect-changes --brief``
...
export default (app: any) => {
  ...
  app.on("file.edited", async ({ $ }: { $: any }) => {
    try {
      await $`code-review-navigator update --skip-flows`.quiet()
    } catch {
  ...
  app.on("session.created", async ({ $ }: { $: any }) => {
    try {
      const result = await $`code-review-navigator status`.quiet()
      const output = result.stdout?.toString().trim()
      if (output) {
        console.log("[code-review-navigator]", output)
      }
    } catch {
  ...
  app.on("tool.execute.before", async (ctx: any) => {
    try {
      ...
      if (typeof cmd === "string" && /^git\\s+commit/i.test(cmd)) {
        const result =
          await ctx.$`code-review-navigator detect-changes --brief`.quiet()
        const output = result.stdout?.toString().trim()
        if (output) {
          console.log("[code-review-navigator] Pre-commit analysis:\\n" + output)
        }
      }
    } catch {
...
def install_opencode_plugin() -> Path:
    """Install the OpenCode user-level plugin for code-review-navigator.

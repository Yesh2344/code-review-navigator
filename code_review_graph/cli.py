"""CLI entry point for code-review-navigator.

Usage:
    code-review-navigator install
    code-review-navigator init
    code-review-navigator uninstall [--dry-run] [--yes] [--repo PATH]
    code-review-navigator build [--base BASE]
    code-review-navigator update [--base BASE]
    code-review-navigator watch
    code-review-navigator status
    code-review-navigator serve [--auto-watch] [--http] [--host ADDR] [--port PORT]
    code-review-navigator mcp [--auto-watch]
    code-review-navigator visualize
    code-review-navigator wiki
    code-review-navigator detect-changes [--base BASE] [--brief]
    code-review-navigator register <path> [--alias name]
    code-review-navigator unregister <path_or_alias>
    code-review-navigator repos
    code-review-navigator daemon start [--foreground]
    code-review-navigator daemon stop
    code-review-navigator daemon restart [--foreground]
    code-review-navigator daemon status
    code-review-navigator daemon logs [--repo ALIAS] [--follow] [--lines N]
    code-review-navigator daemon add <path> [--alias NAME]
    code-review-navigator daemon remove <path_or_alias>
"""
...
if sys.version_info < (3, 10):
    print("code-review-navigator requires Python 3.10 or higher.")
...
def _get_version() -> str:
    ...
    """
    try:
        v = pkg_version("code-review-navigator")
        if v:
            return v
...
def _print_banner() -> None:
    ...
    print(f"""
{c}  ●──●──●{r}
{c}  │╲ │ ╱│{r}       {b}code-review-navigator{r}  {d}v{version}{r}
{c}  ●──{y}◆{c}──●{r}
...
{c}  ●──●──●{r}       {d}smarter code reviews{r}
  ...
  {d}Run{r} {b}code-review-navigator <command> --help{r} {d}for details{r}
""")
...
def _handle_init(args: argparse.Namespace) -> None:
    ...
    if dry_run:
        print("\n[dry-run] Would ensure .gitignore ignores .code-review-navigator/.")
        print("[dry-run] No files were modified.")
        return

    gitignore_state = ensure_repo_gitignore_excludes_crg(repo_root)
    if gitignore_state == "created":
        print("Created .gitignore and added .code-review-navigator/.")
    elif gitignore_state == "updated":
        print("Updated .gitignore with .code-review-navigator/.")
    else:
        print(".gitignore already contains .code-review-navigator/.")
    ...
    print()
    print("Next steps:")
    print("  1. code-review-navigator build    # build the knowledge graph")
...
def main() -> None:
    ...
    uninstall_cmd = sub.add_parser(
        "uninstall",
        help="Safely remove code-review-navigator data, configs, hooks, and generated skills",
    )
    ...
    embed_cmd.add_argument(
        ...
        choices=["local", "openai", "google", "minimax"],
        default=None,
        help="Embedding provider (default: local, needs code-review-navigator[embeddings])",
    )
    embed_cmd.add_argument(
        "--model",
        default=None,
    ...
    if args.version:
        print(f"code-review-navigator {_get_version()}")
        return
    ...
    if args.command in _GRAPH_TOOL_COMMANDS:
        ...
        if not db_path.exists():
            print(
                f"No graph found at {db_path}. Run `code-review-navigator build` first.",
                file=sys.stderr,
            )
            raise SystemExit(1)
    ...
    if args.command == "eval":
        ...
        if getattr(args, "report", False):
            ...
            print("Run 'code-review-navigator eval --report' to generate tables.")
        return
    ...
    if args.command == "uninstall":
        ...
        print("code-review-navigator uninstall — planned actions:")
        _print_report(preview)
        if preview.total_actions == 0:
            if preview.errors:
                raise SystemExit(1)
            print("  (nothing to do — no code-review-navigator artifacts found)")
            return
        if args.dry_run:
    ...
    if args.command in ("register", "unregister", "repos"):
        ...
        if args.command == "register":
            ...
            if not repos:
                ...
                print("Use: code-review-navigator register <path> [--alias name]")
            else:
                for entry in repos:
            ...
            if args.json_output:
                ...
                if stored_branch and current_branch and stored_branch != current_branch:
                    print(
                        ...
                        f"Run 'code-review-navigator build' to rebuild."
                    )
                if vcs == "svn":

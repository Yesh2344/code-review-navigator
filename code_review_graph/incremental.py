    ...
    if not inner_gitignore.exists():
        try:
            ...
            inner_gitignore.write_text(
                "# Auto-generated by code-review-navigator — do not commit database files.\n"
...
def get_data_dir(repo_root: Path) -> Path:
    ...
    3. Default: <repo>/.code-review-navigator/

    By default, ``<repo_root>/.code-review-navigator``. If the
    ...
    if env_override:
        ...
        data_dir = repo_root / ".code-review-navigator"
...
def get_db_path(repo_root: Path) -> Path:
    ...
    legacy top-level ``.code-review-navigator.db`` file into the new
    ...
    legacy_db = repo_root / ".code-review-navigator.db"
    if legacy_db.exists() and not new_db.exists():
        legacy_db.rename(new_db)
    # Discard stale WAL/SHM side-files from the old location
    for suffix in ("-wal", "-shm", "-journal"):
        side = repo_root / f".code-review-navigator.db{suffix}"
        if side.exists():
            side.unlink()
...
def ensure_repo_gitignore_excludes_crg(repo_root: Path) -> str:
    """Ensure repo-level .gitignore excludes ``.code-review-navigator/``.
    ...
    for raw_line in existing.splitlines():
        ...
        if not line or line.startswith("#"):
            continue
        if line == ".code-review-navigator" or line.startswith(".code-review-navigator/"):
            return "already-present"

    block = "# Added by code-review-navigator\n.code-review-navigator/\n"
...
def _load_ignore_patterns(repo_root: Path) -> list[str]:
    """Load ignore patterns from .code-review-navigatorignore file."""
    patterns = list(DEFAULT_IGNORE_PATTERNS)
    ignore_file = repo_root / ".code-review-navigatorignore"

...
_USER_AGENT = (
    f"code-review-navigator/{_crg_version} "
    "(+https://github.com/your-github-username/code-review-navigator)"
)
        ...
        with _MODEL_INIT_LOCK:
            ...
            try:
                ...
                raise ImportError(
                    ...
                    "Run: pip install code-review-navigator[embeddings]"
                )
    ...
    def __init__(self, api_key: str, model: str = "gemini-embedding-001") -> None:
        try:
            ...
            raise ImportError(
                ...
                "Run: pip install code-review-navigator[google-embeddings]"
            )
...
def _warn_cloud_egress(provider_name: str) -> None:
    ...
    if os.environ.get("CRG_ACCEPT_CLOUD_EMBEDDINGS", "").strip() == "1":
        return
    print(
        f"\n⚠️  code-review-navigator: about to embed code via the '{provider_name}' "
        "cloud provider.\n"

<h1 align="center">code-review-navigator</h1>
...
<p align="center">
  <a href="https://pypi.org/project/code-review-navigator/"><img src="https://img.shields.io/pypi/v/code-review-navigator?style=flat-square&color=blue" alt="PyPI"></a>
  <a href="https://pepy.tech/project/code-review-navigator"><img src="https://img.shields.io/pepy/dt/code-review-navigator?style=flat-square" alt="Downloads"></a>
  <a href="https://github.com/your-github-username/code-review-navigator/stargazers"><img src="https://img.shields.io/github/stars/your-github-username/code-review-navigator?style=flat-square" alt="Stars"></a>
  <a href="https://opensource.org/licenses/MIT"><img src="https://img.shields.io/badge/License-MIT-yellow.svg?style=flat-square" alt="MIT Licence"></a>
  <a href="https://github.com/your-github-username/code-review-navigator/actions/workflows/ci.yml"><img src="https://github.com/your-github-username/code-review-navigator/actions/workflows/ci.yml/badge.svg" alt="CI"></a>
  <a href="https://www.python.org/"><img src="https://img.shields.io/badge/python-3.10%2B-blue.svg?style=flat-square" alt="Python 3.10+"></a>
  <a href="https://modelcontextprotocol.io/"><img src="https://img.shields.io/badge/MCP-compatible-green.svg?style=flat-square" alt="MCP"></a>
  <a href="https://code-review-navigator.com"><img src="https://img.shields.io/badge/website-code--review--graph.com-blue?style=flat-square" alt="Website"></a>
...
AI कोडिंग टूल्स रिव्यू टास्क में आपके कोडबेस के बड़े हिस्से दोबारा पढ़ सकते हैं। `code-review-navigator` इस समस्या को हल करता है। यह [Tree-sitter](https://tree-sitter.github.io/tree-sitter/) से आपके कोड का स्ट्रक्चरल मैप बनाता है, बदलावों को इंक्रीमेंटली ट्रैक करता है, और [MCP](https://modelcontextprotocol.io/) के ज़रिए आपके AI असिस्टेंट को सटीक कॉन्टेक्स्ट देता है ताकि वह केवल ज़रूरी कोड ही पढ़े।
...
```bash
pip install code-review-navigator                     # या: pipx install code-review-navigator
code-review-navigator install          # सभी समर्थित प्लेटफ़ॉर्म को स्वचालित रूप से पहचानता और कॉन्फ़िगर करता है
code-review-navigator build            # अपना कोडबेस पार्स करें
```
...
```bash
code-review-navigator install --platform codex       # केवल Codex कॉन्फ़िगर करें
code-review-navigator install --platform cursor      # केवल Cursor कॉन्फ़िगर करें
code-review-navigator install --platform claude-code  # केवल Claude Code कॉन्फ़िगर करें
code-review-navigator install --platform kiro         # केवल Kiro कॉन्फ़िगर करें
```

Python 3.10+ आवश्यक है। सबसे अच्छे अनुभव के लिए [uv](https://docs.astral.sh/uv/) इंस्टॉल करें (MCP कॉन्फ़िग उपलब्ध होने पर `uvx` का उपयोग करेगा, अन्यथा सीधे `code-review-navigator` कमांड पर फ़ॉलबैक करेगा)।
...
<p align="center">
  <img src="diagrams/diagram6_monorepo_funnel.png" alt="Next.js मोनोरिपो: 27,732 फ़ाइलें code-review-navigator से होकर ~15 फ़ाइलों तक — 49 गुना कम टोकन" width="80%" />
</p>
...
सभी आंकड़े 6 वास्तविक ओपन-सोर्स रिपॉज़िटरीज़ (कुल 13 कमिट्स) पर स्वचालित मूल्यांकन रनर से आते हैं। `code-review-navigator eval --all` से पुनः प्राप्त करें। विस्तृत बेंचमार्क डेटा के लिए [अंग्रेज़ी README](README.md) देखें।
...
| **लोकल स्टोरेज** | `.code-review-navigator/` में SQLite फ़ाइल। core graph storage के लिए बाहरी डेटाबेस या क्लाउड सर्विस की ज़रूरत नहीं। |
...
| कमांड | विवरण |
|-------|--------|
| `/code-review-navigator:build-graph` | कोड ग्राफ बनाएं या रीबिल्ड करें |
| `/code-review-navigator:review-delta` | पिछले कमिट के बाद से बदलावों की समीक्षा करें |
| `/code-review-navigator:review-pr` | ब्लास्ट-रेडियस विश्लेषण के साथ पूर्ण PR रिव्यू |
...
```bash
code-review-navigator install          # सभी प्लेटफ़ॉर्म को स्वचालित रूप से पहचानें और कॉन्फ़िगर करें
code-review-navigator install --platform <name>  # किसी विशेष प्लेटफ़ॉर्म को टार्गेट करें
code-review-navigator build            # पूरा कोडबेस पार्स करें
code-review-navigator update           # इंक्रीमेंटल अपडेट (केवल बदली हुई फ़ाइलें)
code-review-navigator status           # ग्राफ़ आंकड़े
code-review-navigator watch            # फ़ाइल बदलाव पर ऑटो-अपडेट
code-review-navigator visualize        # इंटरैक्टिव HTML ग्राफ जनरेट करें
code-review-navigator visualize --format graphml   # GraphML के रूप में एक्सपोर्ट
code-review-navigator visualize --format svg       # SVG के रूप में एक्सपोर्ट
code-review-navigator visualize --format obsidian  # Obsidian वॉल्ट के रूप में एक्सपोर्ट
code-review-navigator visualize --format cypher    # Neo4j Cypher के रूप में एक्सपोर्ट
code-review-navigator wiki             # कम्युनिटीज़ से मार्कडाउन विकी जनरेट करें
code-review-navigator detect-changes   # रिस्क-स्कोर्ड चेंज इम्पैक्ट विश्लेषण
code-review-navigator register <path>  # मल्टी-रिपो रजिस्ट्री में रिपो रजिस्टर करें
code-review-navigator unregister <id>  # रजिस्ट्री से रिपो हटाएं
code-review-navigator repos            # रजिस्टर्ड रिपॉज़िटरीज़ की सूची
code-review-navigator eval             # मूल्यांकन बेंचमार्क चलाएं
code-review-navigator serve            # MCP सर्वर शुरू करें
```
...
इंडेक्सिंग से पथ बाहर करने के लिए, अपनी रिपॉज़िटरी रूट में `.code-review-navigatorignore` फ़ाइल बनाएं:
...
नोट: git रिपॉज़ में, केवल ट्रैक की गई फ़ाइलें इंडेक्स होती हैं (`git ls-files`), इसलिए gitignore की गई फ़ाइलें स्वचालित रूप से छोड़ दी जाती हैं। `.code-review-navigatorignore` का उपयोग ट्रैक की गई फ़ाइलों को बाहर करने या git उपलब्ध न होने पर करें।

वैकल्पिक डिपेंडेंसी ग्रुप:

```bash
pip install "code-review-navigator[embeddings]"          # लोकल वेक्टर एम्बेडिंग (sentence-transformers)
pip install "code-review-navigator[google-embeddings]"   # Google Gemini एम्बेडिंग
pip install "code-review-navigator[communities]"         # कम्युनिटी डिटेक्शन (igraph)
pip install "code-review-navigator[enrichment]"          # Python call-resolution enrichment (Jedi)
pip install "code-review-navigator[eval]"                # मूल्यांकन बेंचमार्क (matplotlib)
pip install "code-review-navigator[wiki]"                # LLM सारांश के साथ विकी जनरेशन (ollama)
pip install "code-review-navigator[all]"                 # सभी वैकल्पिक डिपेंडेंसीज़
```
...
```bash
git clone https://github.com/your-github-username/code-review-navigator.git
cd code-review-navigator
...
<p align="center">
<br>
<a href="https://code-review-navigator.com">code-review-navigator.com</a><br><br>
<code>pip install code-review-navigator && code-review-navigator install</code><br>

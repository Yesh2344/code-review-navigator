...
export class CliWrapper {
    ...
    /**
     * Install the `code-review-navigator` package using the specified installer.
     */
    async installBackend(installer: 'uv' | 'pipx' | 'pip'): Promise<CliResult> {
        const commandMap: Record<typeof installer, { bin: string; args: string[] }> = {
            uv: { bin: 'uv', args: ['pip', 'install', 'code-review-navigator'] },
            pipx: { bin: 'pipx', args: ['install', 'code-review-navigator'] },
            pip: { bin: 'pip3', args: ['install', 'code-review-navigator'] },
        };
    ...
    private getCliPath(): string {
        const configured = vscode.workspace
            ...
            .get<string>('cliPath', '');
        return configured || 'code-review-navigator';
    }
...
function toCliResult(err: unknown): CliResult {
    ...
    if (isEnoent(err)) {
        return {
            success: false,
            stdout: '',
            stderr: 'CLI binary not found. Is code-review-navigator installed?',
        };
    }

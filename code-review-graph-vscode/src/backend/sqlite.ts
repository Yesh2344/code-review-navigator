/**
 * Read-only SQLite reader for the code-review-navigator database.
 *
...
try {
  ...
  if (isAbiMismatch) {
    console.error(
      '[code-review-navigator] better-sqlite3 ABI mismatch. '
      + 'Your VS Code uses a different Node.js version than the one '
      + 'this extension was built for. '
      + 'Try: cd ~/.vscode/extensions/code-review-navigator-* && npm rebuild better-sqlite3'
    );
  }
  throw err;
}

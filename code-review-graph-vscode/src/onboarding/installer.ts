 ...
 *
 * Checks whether the `code-review-navigator` CLI is available and, if not,
...
export class Installer {
    ...
    async checkAndPrompt(): Promise<boolean> {
        ...
        if (selection === 'Manual Instructions') {
            ...
            terminal.sendText('echo "  pip install code-review-navigator"');
            terminal.sendText('echo ""');
            terminal.sendText('echo "Option 2: Install with pipx (recommended)"');
            terminal.sendText('echo "  pipx install code-review-navigator"');
            terminal.sendText('echo ""');
    ...
    async autoInstall(): Promise<boolean> {
        ...
        await vscode.window.withProgress(
            {
                ...
                title: `Installing code-review-navigator via ${installer}...`,
                cancellable: false,
            },
            async () => {
                try {
        ...
        vscode.window.showErrorMessage(
            `Failed to install code-review-navigator via ${installer}. ` +
            'Check the terminal output for details or try installing manually: ' +
            `\`${installer} install code-review-navigator\``,
        );

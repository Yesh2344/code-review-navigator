 ...
 * Checks `.code-review-navigator/graph.db` first, then falls back to `.code-review-navigator.db`.
 */
function findGraphDb(workspaceRoot: string): string | undefined {
  const primary = path.join(workspaceRoot, ".code-review-navigator", "graph.db");
  if (fs.existsSync(primary)) {
    return primary;
  }

  const fallback = path.join(workspaceRoot, ".code-review-navigator.db");
    ...
    vscode.commands.registerCommand("codeReviewGraph.embedGraph", async () => {
      ...
      if (result.success) {
        ...
        const msg = result.stderr.includes("not installed")
          ? "Install embeddings support: pip install code-review-navigator[embeddings]"
...
function watchGraphDb(context: vscode.ExtensionContext): void {
  const watcher = vscode.workspace.createFileSystemWatcher(
    "**/.code-review-navigator/graph.db"
  );

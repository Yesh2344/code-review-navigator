...
AI 编码工具在审查任务中可能会反复读取代码库的大量内容。`code-review-navigator` 解决了这个问题。它使用 [Tree-sitter](https://tree-sitter.github.io/tree-sitter/) 构建代码的结构化映射，增量跟踪变更，并通过 [MCP](https://modelcontextprotocol.io/) 为 AI 助手提供精准的上下文，使其只读取真正需要的内容。
...
```bash
pip install code-review-navigator                     # 或: pipx install code-review-navigator
code-review-navigator install          # 自动检测并配置所有支持的平台
code-review-navigator build            # 解析代码库
```
...
```bash
code-review-navigator install --platform codex       # 仅配置 Codex
code-review-navigator install --platform cursor      # 仅配置 Cursor
code-review-navigator install --platform claude-code  # 仅配置 Claude Code
code-review-navigator install --platform kiro         # 仅配置 Kiro
```

需要 Python 3.10+。为获得最佳体验，建议安装 [uv](https://docs.astral.sh/uv/)（如果可用，MCP 配置将使用 `uvx`，否则直接使用 `code-review-navigator` 命令）。
...
<p align="center">
  <img src="diagrams/diagram6_monorepo_funnel.png" alt="code-review-navigator 仓库：208,821 个源码 token 收敛为约 2,495 token 的图响应——每个问题的 token 减少 93 倍" width="80%" />
</p>
...
所有数据来自针对 6 个真实开源仓库（共 13 次提交）的自动化评估。可通过 `code-review-navigator eval --all` 复现。完整基准测试数据请参阅[英文 README](README.md)。
...
| **本地存储** | SQLite 文件存储在 `.code-review-navigator/` 中，核心图存储无需外部数据库或云服务 |
| **监听模式** | 工作时持续更新图 |
...
| 命令 | 说明 |
|------|------|
| `/code-review-navigator:build-graph` | 构建或重新构建代码图 |
| `/code-review-navigator:review-delta` | 审查自上次提交以来的变更 |
| `/code-review-navigator:review-pr` | 完整的 PR 审查，含影响半径分析 |
...
```bash
code-review-navigator install          # 自动检测并配置所有平台
code-review-navigator install --platform <name>  # 指定特定平台
code-review-navigator build            # 解析整个代码库
code-review-navigator update           # 增量更新（仅变更文件）
code-review-navigator status           # 图统计信息
code-review-navigator watch            # 文件变更时自动更新
code-review-navigator visualize        # 生成交互式 HTML 图
code-review-navigator visualize --format graphml   # 导出为 GraphML
code-review-navigator visualize --format svg       # 导出为 SVG
code-review-navigator visualize --format obsidian  # 导出为 Obsidian 知识库
code-review-navigator visualize --format cypher    # 导出为 Neo4j Cypher
code-review-navigator wiki             # 从社区结构生成 Markdown Wiki
code-review-navigator detect-changes   # 风险评分的变更影响分析
code-review-navigator register <path>  # 将仓库注册到多仓库注册表
code-review-navigator unregister <id>  # 从注册表移除仓库
code-review-navigator repos            # 列出已注册的仓库
code-review-navigator eval             # 运行评估基准测试
code-review-navigator serve            # 启动 MCP 服务器
```
...
要排除特定路径不被索引，请在仓库根目录创建 `.code-review-navigatorignore` 文件：
...
注意：在 git 仓库中，仅索引已跟踪的文件（`git ls-files`），因此 gitignore 中的文件会自动跳过。`.code-review-navigatorignore` 用于排除已跟踪的文件，或在没有 git 的环境中使用。

可选依赖组：

```bash
pip install "code-review-navigator[embeddings]"          # 本地向量嵌入 (sentence-transformers)
pip install "code-review-navigator[google-embeddings]"   # Google Gemini 嵌入
pip install "code-review-navigator[communities]"         # 社区检测 (igraph)
pip install "code-review-navigator[enrichment]"          # Python 调用解析增强 (Jedi)
pip install "code-review-navigator[eval]"                # 评估基准测试 (matplotlib)
pip install "code-review-navigator[wiki]"                # 使用 LLM 摘要生成 Wiki (ollama)
pip install "code-review-navigator[all]"                 # 所有可选依赖
```
...
>
> 另外请注意：目前 `code-review-navigator` 只嵌入**函数签名**（每节点约 10 tokens，例如 `"parse_file function (path: str) returns Tree"`）。那些靠长 context 理解函数 body 来拉开差距的模型（Gemini 2 或 Qwen3-8B 在 MTEB-code 的 SOTA 分数）在这个输入长度下跟小模型的品质差距会小很多。Body / docstring 嵌入已列为后续增强任务。

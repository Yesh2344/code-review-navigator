...
def test_relativize_falls_back_to_repo_segment_without_env(monkeypatch):
    ...
    monkeypatch.setenv("GITHUB_REPOSITORY", "owner/code-review-navigator")
    out = render.relativize_path(
        "/home/runner/work/code-review-navigator/code-review-navigator/"
...
def test_footer_powered_by(report):
    body = render.render_markdown(report)
    assert "Powered by [code-review-navigator]" in body
...
def test_cli_subprocess_stdout():
    ...
    assert "Powered by [code-review-navigator]" in result.stdout

...
class TestConfigParsing:
    ...
    def test_load_config_no_git_dir(self, tmp_path):
        """Repos without .git or .code-review-navigator are skipped."""
...
class TestWatchDaemon:
    ...
    def daemon_env(self, tmp_path):
        ...
        repo_a.mkdir()
        (repo_a / ".git").mkdir()
        (repo_a / ".code-review-navigator").mkdir()
        # Create graph.db so _initial_build is skipped
        (repo_a / ".code-review-navigator" / "graph.db").touch()

        repo_b = tmp_path / "repo-b"
        repo_b.mkdir()
        (repo_b / ".git").mkdir()
        (repo_b / ".code-review-navigator").mkdir()
        (repo_b / ".code-review-navigator" / "graph.db").touch()
    ...
    def test_reconcile_add(self, daemon_env):
        ...
        beta_db = Path(config.repos[1].path) / ".code-review-navigator" / "graph.db"
        beta_db.unlink()

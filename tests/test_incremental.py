...
class TestGetDbPath:
    def test_creates_directory_and_db_path(self, tmp_path):
        ...
        assert db_path == tmp_path / ".code-review-navigator" / "graph.db"
        assert (tmp_path / ".code-review-navigator").is_dir()

    def test_creates_gitignore(self, tmp_path):
        get_db_path(tmp_path)
        gi = tmp_path / ".code-review-navigator" / ".gitignore"
        assert gi.exists()
        assert "*\n" in gi.read_text()

    def test_migrates_legacy_db(self, tmp_path):
        legacy = tmp_path / ".code-review-navigator.db"
    ...
    def test_cleans_legacy_side_files(self, tmp_path):
        ...
        for suffix in ("-wal", "-shm", "-journal"):
            (tmp_path / f".code-review-navigator.db{suffix}").write_text("side")
        get_db_path(tmp_path)
        for suffix in ("-wal", "-shm", "-journal"):
            assert not (tmp_path / f".code-review-navigator.db{suffix}").exists()
...
class TestEnsureRepoGitignoreExcludesCrg:
    ...
    def test_idempotent_when_present(self, tmp_path):
        gitignore = tmp_path / ".gitignore"
        gitignore.write_text(".code-review-navigator/\n")

        state = ensure_repo_gitignore_excludes_crg(tmp_path)
        assert state == "already-present"
        assert gitignore.read_text() == ".code-review-navigator/\n"
...
class TestIgnorePatterns:
    ...
    def test_custom_ignore_file(self, tmp_path):
        ignore = tmp_path / ".code-review-navigatorignore"
...
class TestDataDir:
    ...
    def test_default_uses_repo_subdir(self, tmp_path, monkeypatch):
        """Without CRG_DATA_DIR, graphs live at <repo>/.code-review-navigator."""
        ...
        assert result == tmp_path / ".code-review-navigator"
    ...
    def test_env_override_replaces_repo_subdir(self, tmp_path, monkeypatch):
        """CRG_DATA_DIR replaces the default <repo>/.code-review-navigator."""
        ...
        assert result.is_dir()
        # The repo itself should NOT have a .code-review-navigator dir now
        assert not (repo / ".code-review-navigator").exists()
...
class TestDataDirRegistry:
    ...
    def test_registry_data_dir_overrides_default(self, tmp_path, monkeypatch):
        """Registry data_dir should override default .code-review-navigator."""
        from code_review_graph.incremental import get_data_dir
    ...
    def test_registry_fallback_to_default(self, tmp_path, monkeypatch):
        ...
        # Don't set in registry
        result = get_data_dir(repo)
        assert result == repo / ".code-review-navigator"
        assert result.is_dir()

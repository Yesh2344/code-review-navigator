...
def _seed_repo_relative_graph(root: Path) -> None:
    ...
    graph_dir = root / ".code-review-navigator"
    graph_dir.mkdir()
...
class TestRepoRootValidation:
    ...
    def test_validate_repo_root_error_mentions_svn_marker(self, tmp_path):
        with pytest.raises(ValueError, match=r"\.git, \.svn, or \.code-review-navigator"):
...
class TestQueryGraphTestsFor:
    ...
    def setup_method(self):
        ...
        # _validate_repo_root requires .git or .code-review-navigator.
        (self.repo_root / ".code-review-navigator").mkdir()
...
class TestGetDocsSection:
    ...
    def test_explicit_repo_root_uses_that_docs_file(self, tmp_path):
        (tmp_path / ".code-review-navigator").mkdir()
...
class TestFlowTools:
    ...
    def setup_method(self):
        """Set up a temp dir with .git and .code-review-navigator, seed data, build flows."""
        self.tmp_dir = tempfile.mkdtemp()
        ...
        db_path = str(self.root / ".code-review-navigator" / "graph.db")
...
class TestCommunityTools:
    ...
    def setup_method(self):
        """Set up a temp dir with .git and .code-review-navigator, seed clustered graph."""
        self.tmp_dir = tempfile.mkdtemp()
        self.root = Path(self.tmp_dir).resolve()
...
class TestGetMinimalContext:
    ...
    def setup_method(self):
        ...
        db_path = self.root / ".code-review-navigator" / "graph.db"
...
class TestGraphProvenance:
    ...
    def _make_repo(tmp_path, metadata=None, name="repo"):
        ...
        repo.mkdir(parents=True)
        (repo / ".git").mkdir()
        graph_dir = repo / ".code-review-navigator"
        graph_dir.mkdir()
        store = GraphStore(graph_dir / "graph.db")
        try:
            store.upsert_node(NodeInfo(
    ...
    def test_exclusive_lock_fails_soft_promptly(self, tmp_path):
        ...
        db_path = repo / ".code-review-navigator" / "graph.db"

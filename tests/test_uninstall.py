...
def test_uninstall_removes_mcp_entry_for_every_current_platform_spec(
    ...
    if spec["format"] == "toml":
        _write(
            ...
            "command = \"code-review-navigator\"\n\n"
            "[mcp_servers.other]\ncommand = \"other\"\n",
        )
    else:
        if spec["format"] == "array":
            container: object = [
                {"name": "code-review-navigator", "command": "code-review-navigator"},
                {"name": "other", "url": "https://example.test/mcp"},
            ]
        else:
            container = {
                "code-review-navigator": {"command": "code-review-navigator"},
    ...
    if spec["format"] == "toml":
        ...
        assert "[mcp_servers.code-review-navigator]" not in text
...
def test_platform_inventory_is_derived_not_hard_coded(
    ...
    _write_json(config, {"servers": {"code-review-navigator": {}, "mine": {}}})
...
def test_source_pr_legacy_mcp_paths_remain_supported(
    ...
    for path in (repo_legacy, user_legacy):
        _write_json(
            path,
            {"mcpServers": {"code-review-navigator": {}, "other": {}}},
        )
...
def test_jsonc_comments_trailing_commas_and_https_survive(
    ...
    assert "code-review-navigator" not in raw
...
def test_only_installer_owned_gitignore_block_is_removed(
    ...
    _write(
        gitignore,
        "dist/\n# Added by code-review-navigator\n.code-review-navigator/\ncoverage/\n",
    )
...
def test_dry_run_is_meaningful_and_byte_for_byte_read_only(
    ...
    _write_json(config, {"mcpServers": {"code-review-navigator": {}}})
    data = fake_repo / ".code-review-navigator" / "graph.db"
    data.parent.mkdir()
...
def test_failed_atomic_config_write_preserves_original_bytes(
    ...
    _write_json(
        config,
        {"mcpServers": {"code-review-navigator": {}, "other": {"command": "mine"}}},
    )
...
def test_atomic_config_replace_preserves_file_mode(
    ...
    config = fake_repo / ".mcp.json"
    _write_json(config, {"mcpServers": {"code-review-navigator": {}, "other": {}}})
    config.chmod(0o640)
...
def test_non_repository_directory_is_refused_without_deleting_data(
    ...
    data = ordinary_directory / ".code-review-navigator" / "unrelated.txt"
    ...
    assert _read_jsonc(config) == {"mcpServers": {"code-review-navigator": {}}}
...
def test_symlink_and_out_of_boundary_paths_are_skipped(
    ...
    os.symlink(outside_data, fake_repo / ".code-review-navigator", target_is_directory=True)

    outside_config = tmp_path / "outside-config.json"
    _write_json(outside_config, {"servers": {"code-review-navigator": {}, "other": {}}})
    monkeypatch.setitem(
        skills.PLATFORMS,
        "malicious-path",
        {
    ...
    assert (fake_repo / ".code-review-navigator").is_symlink()
...
def test_malformed_config_is_unchanged_and_other_cleanup_continues(
    ...
    _write_json(valid, {"mcpServers": {"code-review-navigator": {}, "other": {}}})
...
def test_partial_filesystem_failure_is_reported_and_does_not_stop_cleanup(
    ...
    blocked = fake_repo / ".code-review-navigator.db"
    blocked.write_bytes(b"db")
    removable = fake_repo / ".code-review-navigator.db-wal"
...
def test_keep_flags_preserve_data_and_user_configuration(
    ...
    repo_data = fake_repo / ".code-review-navigator"
    repo_data.mkdir()
    (repo_data / "graph.db").write_bytes(b"db")
    legacy = fake_repo / ".code-review-navigator.db"
    legacy.write_bytes(b"db")
    user_data = fake_home / ".code-review-navigator"
    user_data.mkdir()
    (user_data / "registry.json").write_text("{}", encoding="utf-8")
    user_config = fake_home / ".qwen" / "settings.json"
    _write_json(user_config, {"mcpServers": {"code-review-navigator": {}}})
    ...
    assert "code-review-navigator" in _read_jsonc(user_config)["mcpServers"]
...
def test_all_repos_reads_registry_before_removing_user_data(
    ...
    _write_json(registered_config, {"mcpServers": {"code-review-navigator": {}}})
    external_data = tmp_path / "external-data"
    external_data.mkdir()
    (external_data / "graph.db").write_bytes(b"keep")
    registry_dir = fake_home / ".code-review-navigator"
    registry_dir.mkdir()
    _write_json(
...
def test_cli_dry_run_and_confirmation_are_safe(
    ...
    with patch.object(
        sys,
        "argv",
        ["code-review-navigator", "uninstall", "--repo", str(fake_repo), "--dry-run"],
    ):
        cli.main()
    assert "dry-run" in capsys.readouterr().out.lower()
    assert "code-review-navigator" in config.read_text(encoding="utf-8")

    with (
        patch.object(
            sys,
            "argv",
            ["code-review-navigator", "uninstall", "--repo", str(fake_repo)],
        ),

        ...
        for hook_type, entries in config["hooks"].items():
            for entry in entries:
                for hook in entry["hooks"]:
                    assert "command -v code-review-navigator" in hook["command"], (
...
class TestInstallGitHook:
    ...
    def test_creates_executable_pre_commit_hook(self, tmp_path):
        ...
        assert content.startswith("#!/")
        assert "code-review-navigator detect-changes" in content
    ...
    def test_idempotent(self, tmp_path):
        ...
        assert content.count("code-review-navigator detect-changes") == 1
    ...
    def test_real_repo_installs_into_git_hooks(self, tmp_path):
        ...
        assert os.access(hook_path, os.X_OK)
        assert "code-review-navigator detect-changes" in hook_path.read_text()
...
class TestGenerateCodexHooksConfig:
    ...
    def test_has_post_tool_use(self, tmp_path):
        ...
        assert "update" in inner["command"]
        assert inner["command"].startswith("cat >/dev/null || true; ")
        assert inner["statusMessage"] == "Updating code-review-navigator"
    ...
    def test_has_session_start(self, tmp_path):
        ...
        assert "status" in inner["command"]
        assert inner["command"].startswith("cat >/dev/null || true; ")
        assert inner["statusMessage"] == "Checking code-review-navigator status"
    ...
    def test_commands_do_not_pin_a_specific_repo_path(self, tmp_path):
        ...
        assert "code-review-navigator update --skip-flows" in post_cmd
        assert "code-review-navigator status" in session_cmd
...
class TestInjectPlatformInstructionsFiltering:
    def test_all_writes_every_file(self, tmp_path):
        ...
        assert set(updated) == {
            ...
            "QODER.md", ".kiro/steering/code-review-navigator.md",
            ".github/code-review-navigator.instruction.md",
            "CODEBUDDY.md",
        }
    ...
    def test_claude_writes_nothing(self, tmp_path):
        ...
        assert not (tmp_path / ".github" / "code-review-navigator.instruction.md").exists()
...
class TestCodeBuddyPlatform:
    ...
    def test_install_preserves_jsonc_content(self, tmp_path):
        ...
        assert data["mcpServers"]["code-review-navigator"]["type"] == "stdio"
    ...
    def test_all_dedupes_only_claude_and_codebuddy_shared_contract(
        ...
        assert "code-review-navigator" in data["mcpServers"]
        assert "code-review-navigator" in data["servers"]
    ...
    def test_project_hooks_preserve_user_settings_and_resolve_repo_at_runtime(
        ...
        installed = [
            ...
            for entry in entries
            for hook in entry["hooks"]
            if "code-review-navigator" in hook.get("command", "")
        ]
        assert installed
        for hook in installed:
            command = hook["command"]
            assert "command -v code-review-navigator" in command
        ...
        crg_entry = next(
            ...
            if any("code-review-navigator" in hook.get("command", "") for hook in entry["hooks"])
        )
...
class TestInstallPlatformConfigs:
    ...
    def test_install_codex_config(self, tmp_path):
        ...
        entry = data["mcp_servers"]["code-review-navigator"]
    ...
    def test_install_codex_preserves_existing_toml(self, tmp_path):
        ...
        assert data["mcp_servers"]["code-review-navigator"]["command"] == expected_cmd
    ...
    def test_install_codex_no_duplicate(self, tmp_path):
        ...
        codex_config.write_text(
            "\n".join(
                [
                    "[mcp_servers.code-review-navigator]",
                    'command = "uvx"',
                    'args = ["code-review-navigator", "serve"]',
                    'type = "stdio"',
                    "",
                ]
            ),
        ...
        with patch.dict(
            ...
            install_platform_configs(tmp_path, target="codex")
        assert codex_config.read_text().count("[mcp_servers.code-review-navigator]") == 1
    ...
    def test_install_windsurf_config(self, tmp_path):
        ...
        data = json.loads(config_path.read_text())
        entry = data["mcpServers"]["code-review-navigator"]
    ...
    def test_install_zed_config(self, tmp_path):
        ...
        assert "code-review-navigator" in data["context_servers"]
    ...
    def test_install_continue_config(self, tmp_path):
        ...
        assert data["mcpServers"][0]["name"] == "code-review-navigator"
    ...
    def test_install_opencode_config(self, tmp_path):
        ...
        data = json.loads(config_path.read_text())
        entry = data["mcp"]["code-review-navigator"]
    ...
    def test_install_opencode_prefers_existing_jsonc_and_preserves_servers(self, tmp_path):
        ...
        assert "code-review-navigator" in data["mcp"]
    ...
    def test_install_opencode_warns_about_legacy_dotfile(self, tmp_path, capsys):
        ...
        legacy.write_text(
            json.dumps({"mcpServers": {"code-review-navigator": {"command": "uvx"}}}),
            encoding="utf-8",
        )
    ...
    def test_continue_array_no_duplicate(self, tmp_path):
        ...
        existing = {
            "mcpServers": [{"name": "code-review-navigator", "command": "uvx", "args": ["serve"]}]
        }
    ...
    def test_install_qoder_config(self, tmp_path):
        ...
        assert data["mcpServers"]["code-review-navigator"]["type"] == "stdio"
        expected_cmd, _ = _detect_serve_command()
        assert data["mcpServers"]["code-review-navigator"]["command"] == expected_cmd
...
class TestCursorHookScripts:
    ...
    def test_update_script_runs_update(self):
        scripts = _cursor_hook_scripts()
        assert "code-review-navigator update --skip-flows" in scripts["crg-update.sh"]

    def test_session_start_script_runs_status(self):
        scripts = _cursor_hook_scripts()
        assert "code-review-navigator status" in scripts["crg-session-start.sh"]

    def test_pre_commit_script_runs_detect_changes(self):
        scripts = _cursor_hook_scripts()
        assert "code-review-navigator detect-changes --brief" in scripts["crg-pre-commit.sh"]
...
class TestKiroPlatform:
    ...
    def test_install_kiro_preserves_existing_servers(self, tmp_path):
        """Existing mcpServers entries are preserved when adding code-review-navigator."""
        config_path = tmp_path / ".kiro" / "settings" / "mcp.json"
        config_path.parent.mkdir(parents=True)
        config_path.write_text(
            json.dumps({"mcpServers": {"other-server": {"command": "other"}}}),
    ...
    def test_install_kiro_no_duplicate(self, tmp_path):
        """Second install skips when code-review-navigator already exists."""
        install_platform_configs(tmp_path, target="kiro")
        config_path = tmp_path / ".kiro" / "settings" / "mcp.json"
        ...
        assert list(data["mcpServers"].keys()).count("code-review-navigator") == 1

    def test_kiro_steering_file_written(self, tmp_path):
        """inject_platform_instructions creates .kiro/steering/code-review-navigator.md."""
        updated = inject_platform_instructions(tmp_path, target="kiro")
        assert ".kiro/steering/code-review-navigator.md" in updated
        steering = tmp_path / ".kiro" / "steering" / "code-review-navigator.md"
    ...
    def test_kiro_steering_idempotent(self, tmp_path):
        ...
        first = (tmp_path / ".kiro" / "steering" / "code-review-navigator.md").read_text()
        inject_platform_instructions(tmp_path, target="kiro")
        second = (tmp_path / ".kiro" / "steering" / "code-review-navigator.md").read_text()
        assert first == second
...
class TestCopilotPlatform:
    ...
    def test_install_copilot_config(self, tmp_path):
        ...
        data = json.loads(config_path.read_text())
        assert "code-review-navigator" in data["servers"]
        entry = data["servers"]["code-review-navigator"]
        assert entry["type"] == "stdio"
        assert "serve" in entry["args"]

    def test_install_copilot_preserves_existing_servers(self, tmp_path):
        """Existing server entries are preserved when adding code-review-navigator."""
        config_path = tmp_path / ".vscode" / "mcp.json"
        config_path.parent.mkdir(parents=True)
        config_path.write_text(
    ...
    def test_install_copilot_no_duplicate(self, tmp_path):
        ...
        assert first_content == second_content
        data = json.loads(second_content)
        assert list(data["servers"].keys()).count("code-review-navigator") == 1

    def test_copilot_instructions_file_written(self, tmp_path):
        """inject_platform_instructions creates .github/code-review-navigator.instruction.md."""
        updated = inject_platform_instructions(tmp_path, target="copilot")
        assert ".github/code-review-navigator.instruction.md" in updated
        instructions = tmp_path / ".github" / "code-review-navigator.instruction.md"
    ...
    def test_copilot_instructions_idempotent(self, tmp_path):
        ...
        first = (tmp_path / ".github" / "code-review-navigator.instruction.md").read_text()
        inject_platform_instructions(tmp_path, target="copilot")
        second = (tmp_path / ".github" / "code-review-navigator.instruction.md").read_text()
        assert first == second
    ...
    def test_copilot_writes_only_copilot_instructions(self, tmp_path):
        ...
        updated = inject_platform_instructions(tmp_path, target="copilot")
        assert updated == [".github/code-review-navigator.instruction.md"]
        assert not (tmp_path / "AGENTS.md").exists()
        assert not (tmp_path / "GEMINI.md").exists()
        assert not (tmp_path / ".cursorrules").exists()
        assert not (tmp_path / ".windsurfrules").exists()
...
class TestDetectServeCommand:
    ...
    def test_poetry_active_returns_poetry_run(self, monkeypatch):
        ...
        assert cmd == "poetry"
        assert args == ["run", "code-review-navigator", "serve"]
    ...
    def test_uvx_fallback(self, monkeypatch):
        ...
        cmd, args = _detect_serve_command()
        assert cmd == "uvx"
        assert args == ["code-review-navigator", "serve"]
...
class TestOpenCodePluginContent:
    ...
    def test_hooks_file_edited_event(self):
        ...
        assert "code-review-navigator update --skip-flows" in content

    def test_hooks_session_created_event(self):
        content = _opencode_plugin_content()
        assert '"session.created"' in content
        assert "code-review-navigator status" in content

    def test_hooks_tool_execute_before_event(self):
        content = _opencode_plugin_content()
        assert '"tool.execute.before"' in content
        assert "code-review-navigator detect-changes --brief" in content
...
class TestGeneratedHooksGuardGitRepo:
    ...
    def test_post_tool_use_command_guarded_by_git_check(self):
        ...
        idx_update = cmd.index("code-review-navigator update")
    ...
    def test_session_start_command_guarded_by_git_check(self):
        ...
        assert "git rev-parse --git-dir" in cmd
        idx_guard = cmd.index("git rev-parse --git-dir")
        idx_status = cmd.index("code-review-navigator status")
